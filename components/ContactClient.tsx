    ...
    <section id="contact" style={{ padding: "5rem 2rem", background: "#fff", borderTop: "1px solid rgba(0,0,0,0.06)" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: "3rem" }}>
          ...
          <h2 style={{ fontSize: "clamp(1.8rem, 3vw, 2.5rem)", fontWeight: 800, fontFamily: "Plus Jakarta Sans, sans-serif" }}>{heading}</h2>
          <p style={{ color: "#6b7280", marginTop: "0.5rem" }}>{subheading}</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "4rem", alignItems: "flex-start" }}>
          <div>
            {[
              { icon: MessageCircle, label: "WhatsApp", value: whatsapp, href: `https://wa.me/${whatsapp.replace(/\D/g, "")}`, color: "#25D366" },
              { icon: Mail, label: "Email", value: email, href: `mailto:${email}`, color: "#2563eb" },
              { icon: Phone, label: "Phone", value: phone, href: `tel:${phone.replace(/\D/g, "")}`, color: "#7c3aed" },
            ].map(ci => {
              const Icon = ci.icon;
              return (
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <button type="submit" disabled={submitting} style={{
                  background: submitting ? "#9ca3af" : "#2563eb", border: "none", borderRadius: "10px", padding: "13px",
