    ...
    <section id="work" style={{ padding: "5rem 2rem", background: "#f8f9fb", borderTop: "1px solid rgba(0,0,0,0.06)" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: "3rem" }}>
          ...
          <p style={{ color: "#6b7280" }}>{subheading}</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "20px" }}>
          {filtered.map((p) => {
            const color = colorMap[p.color] || colorMap.blue;
            return (
              <div key={p.id} style={{
              ...
              >
                ...
                <div style={{ padding: "1.5rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "1rem" }}>
                    <span style={{ fontSize: "11px", fontWeight: 700, padding: "3px 10px", borderRadius: "999px", background: color + "15", color }}>{p.category}</span>
                  ...
                  <div style={{ display: "inline-block", padding: "6px 14px", background: color + "10", border: `1px solid ${color}30`, borderRadius: "8px", fontSize: "13px", fontWeight: 700, color }}>{p.result}</div>
                </div>
              </div>
            );
          })}
